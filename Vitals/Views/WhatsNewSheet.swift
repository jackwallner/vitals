...
/// that introduced the opt-in Vitals+ extras. It is purely an awareness surface:
/// the core Today view and every default are unchanged, and nothing here flips a
/// feature on. Non-subscribers get a "Try Vitals+ free" path; subscribers get an
...
struct WhatsNewSheet: View {
    ...
    private let features: [PlusFeature] = [.excludedDays, .macros, .projections, .streaks]
            ...
            VStack(spacing: 0) {
                ScrollView(showsIndicators: false) {
                    VStack(spacing: 18) {
                        ...
                        VStack(spacing: 6) {
                            Text("New in Total Calories")
                                .font(.system(.title2, design: .rounded, weight: .bold))
                                .foregroundStyle(Theme.textPrimary)
                                .multilineTextAlignment(.center)
                            Text("Excluded Days joins the optional Vitals+ extras: pick the days that shouldn't count (a sick week, a flight, a day off the wrist) and every average, total, and streak skips them. Everything you already use stays exactly the same, and these are off until you switch them on.")
    ...
    private var hero: some View {
        ZStack {
            Circle()
                ...
                .shadow(color: Theme.caloriesPrimary.opacity(0.4), radius: 14, x: 0, y: 6)
