...
This project owns the simulator device `agent-vitals`. Multiple agents work in
...
```bash
UDID=$(agent-sim boot vitals)        # create if needed + boot headless; prints UDID
...
APP=$(find ~/Library/Developer/Xcode/DerivedData/Vitals-*/Build/Products -maxdepth 2 -name "*.app" -path "*iphonesimulator*" | head -1)
...
agent-sim screenshot vitals          # PNG at /tmp/agent-vitals.png
agent-sim shutdown vitals            # free resources when done
```
